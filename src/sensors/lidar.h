...
struct Lidar
{
	...
	{
		minDistance = 5; //minDistance 5 to remove points from roof of ego car
		maxDistance = 50;
		resoultion = 0.2;

		sderr = 0.2; //Adding a noise or jitter 
		cars = setCars;
		groundSlope = setGroundSlope;

		int numLayers = 8; //higher_resolution 
		// the steepest vertical angle
		double steepestAngle =  30.0*(-pi/180);
		double angleRange = 26.0*(pi/180);
	
		double horizontalAngleInc = pi/64; //higher_resolution
